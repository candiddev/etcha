...
This simple Pattern, built using [`etcha build`](../../guides/building-patterns), will always create a file, `helloworld`, with the current environment hostname, and read the contents of it when it changes, sending the output to the event `build_manifest`.
...
Etcha built our JWT successfully.  It included the output of the `build_manifest` event in the `etchaBuildManifest` property, too!
