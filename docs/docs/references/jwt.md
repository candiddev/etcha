...
Number, a unix timestamp indicating when the JWT expires.  This is populated by the Pattern property [`expiresInSec`]({{< ref "/docs/references/patterns#expiresinsec" >}}).
...
Number, a unix timestamp of when the JWT was created.  This is populated automatically during Pattern build and cannot be set.
...
Number, a unix timestamp of when the JWT becomes valid.  This is populated automatically during Pattern build to the same value as `iat`.  Future Etcha releases may make this configurable.
...
A map of values that will be combined with {{% config vars %}} when the Pattern is rendered.  These are exposed using the [Jsonnet native function, `getConfig`]({{< ref "/docs/references/jsonnet#getConfig" >}}).
...
- It checks the signature using {{% config run_verifyKeys %}}.  If a verify key cannot be found that matches the JWT, it does not accept the JWT (during either push or pull).
...
Organizations wishing to provide more verification/signing capabilities can leverage custom {{% config run_verifyCommands %}} and {{% config build_signCommands %}}.  See [Building Patterns]({{< ref "/docs/guides/building-patterns" >}}) for more information.
