...
We'll configure Etcha to sign Patterns using OpenBao via {{% config build_signingCommands %}}.  Here is an example configuration file:
...
We'll configure Etcha to verify Patterns using OpenBao via {{% config run_verifyCommands %}}.  Here is an example configuration file:
...
We can add the key to {{% config run_verifyKeys %}} by adding the prefix `ed25519public:`
