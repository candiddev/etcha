...
In your code repository, run the command {{% cli init %}}.  This command will do a few things:
...
This is meant to be an easy way for folks to "vendor" the [Etcha standard library]({{% ref "/docs/references/libraries" %}}).
{{% /alert %}}
...
Etcha will flatten lists into a single, ordered list.  If you're curious to see how your Pattern might look once it's rendered, run {{% cli render %}}.
