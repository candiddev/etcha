...
func PushTargets(ctx context.Context, c *config.Config, targets map[string]config.PushTarget, source, cmd string, opts PushOpts) ([]string, errs.Err) { //nolint:gocognit,revive
	...
	for i := range t {
		...
		go func(target string) {
			...
			if res.Err != "" {
				o = append(o, fmt.Sprintf("Error: %s", res.Err))
			}

			if path {
				if len(res.ChangedIDs) > 0 {
					o = append(o, fmt.Sprintf("Changed: %s", strings.Join(res.ChangedIDs, ", ")))
				}

				if len(res.RemovedIDs) > 0 {
					o = append(o, fmt.Sprintf("Removed: %s", strings.Join(res.RemovedIDs, ", ")))
				}
			} else {
	...
	return r.Show(), logger.Error(ctx, terr)
}
