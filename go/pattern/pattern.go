...
func ParsePatternFromImports(ctx context.Context, c *config.Config, configSource string, imports *jsonnet.Imports, runVars map[string]any) (*Pattern, errs.Err) {
	...
	if ct := p.Build.Count(); ct > c.License.Commands {
		return nil, logger.Error(ctx, fmt.Errorf("number of commands (%d) in source %s build exceeds the license amount (%d), please upgrade your license or reduce the number of commands", ct, configSource, c.License.Commands))
	}

	if ct := p.Run.Count(); ct > c.License.Commands {
		return nil, logger.Error(ctx, fmt.Errorf("number of commands (%d) in source %s run exceeds the license amount (%d), please upgrade your license or reduce the number of commands", ct, configSource, c.License.Commands))
	}
